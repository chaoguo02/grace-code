...
| `tools/factory.py` | `BuiltTool` — no `source` field | Add `source: str = "system"` parameter. `mcp_props is not None → source="mcp"`. |
...
| `core/tool_execution.py` | `ToolEvidenceRecorder` — no skill-specific handling | No change. Since `SkillActivationTool` goes through `ToolExecutionPipeline`, evidence is recorded automatically. |
...
4. **Phase 4 Completion Guard migration**: The Completion Guard currently checks `required_skills` by looking at `SKILL_LOADED` evidence entries. After Phase 4, skill activations through the tool path produce `TOOL_CALL_COMPLETED` instead. Should we:
