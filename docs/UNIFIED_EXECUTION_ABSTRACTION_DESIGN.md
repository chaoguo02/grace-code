...
1. **`skill:{name}` vs `Skill:{name}` vs Flat Name**: FINAL DECISION: Flat Original Name + Deprecate Generic Loader. All skills register by their frontmatter-declared original name (e.g., `review`, `web-search`). No prefixes — LLM must not perceive internal source markers. The existing generic "Skill" tool is immediately deprecated: renamed to `__legacy_skill_loader` and excluded from `get_schemas()` via `visible_to_llm=False`. If a skill's original name conflicts with a native tool, Phase 3 priority rules apply (Native > Skill → Skill rejected with WARNING).

2. **Skill Modifier Lifecycle (Blocker #1)**: FINAL DECISION: Dual-Stack Model with Explicit Scope Tag. `PolicyAwareToolRegistry` maintains two independent modifier stacks: `_turn_modifiers: list` and `_run_modifiers: list`. `SkillActivationTool.execute()` → TURN scope, pushed to `_turn_modifiers`, auto-popped in `after_tool_use` hook. Preload/CLI/HTTP → RUN scope, pushed to `_run_modifiers`, cleaned at end-of-run. Visibility merge: `effective = _run_modifiers + _turn_modifiers` (turn overrides run on conflict).
